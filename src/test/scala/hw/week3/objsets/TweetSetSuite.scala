...
class TweetSetSuite extends FunSuite {
  ...
  test("mostRetweeted: in set5 is Tweet d") {
    new TestSets {
      assert(set5.mostRetweeted == d)
    }
  }

  test("mostRetweeted: in set4c is Tweet c") {
    new TestSets {
      assert(set4c.mostRetweeted == c)
    }
  }

  test("descending: set3") {
    new TestSets {
      println("--------------->")
  ...
  test("descending: set5") {
    new TestSets {
      ...
      assert(trends.head.user == "d" || trends.head.user == "c")
    }
  }
